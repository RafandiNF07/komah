...
export function generateOrderReceipt(order) {
  ...
  // Menggunakan currentY dinamis agar posisi komponen otomatis menyesuaikan baris di atasnya
  let currentY = 52;
  const lineHeight = 7;
  const maxWidth = 170; // Batas lebar teks (190 margin kanan - 20 margin kiri)
  const info = [
  ...
  // --- DETAIL LOKASI (DENGAN AUTO-WRAP) ---
  currentY += 5; // Beri sedikit space dari info di atas
  doc.setFont('helvetica', 'bold');
  doc.text('Detail Lokasi', 20, currentY);
  currentY += lineHeight;

  doc.setFont('helvetica', 'normal');

  // Potong alamat penjemputan otomatis jika terlalu panjang
  const pickupText = `Penjemputan: ${order.pickup_location || '-'}`;
  const pickupLines = doc.splitTextToSize(pickupText, maxWidth);
  doc.text(pickupLines, 20, currentY);
  // Tambahkan currentY secara dinamis berdasarkan jumlah baris yang dihasilkan
  currentY += pickupLines.length * lineHeight; 

  // Potong alamat tujuan otomatis jika ada
  if (order.destination_location) {
    const destText = `Tujuan: ${order.destination_location}`;
    const destLines = doc.splitTextToSize(destText, maxWidth);
    doc.text(destLines, 20, currentY);
    currentY += destLines.length * lineHeight;
  }


  // --- DETAIL LAYANAN ---
  if (order.service_details && Object.keys(order.service_details).length > 0) {
    currentY += 5; // Beri space jarak
    doc.setFont('helvetica', 'bold');
    doc.text('Detail Layanan', 20, currentY);
    currentY += lineHeight;
    doc.setFont('helvetica', 'normal');
    ...
    Object.entries(order.service_details).forEach(([key, value]) => {
      ...
      const serviceText = `${label}: ${displayValue}`;
      const serviceLines = doc.splitTextToSize(serviceText, maxWidth);
      doc.text(serviceLines, 20, currentY);
      currentY += serviceLines.length * lineHeight;
    });
  }

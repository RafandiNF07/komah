  ...
  return (
    <div className="w-full max-w-[850px] mx-auto pb-4">
        ...
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-xl flex items-center justify-center bg-purple/20">
            <Image
          ...
          <div>
            <h1 className="font-headline-md text-[24px] md:text-[28px] font-bold text-text-primary">
              Layanan <span className="text-purple">Delivery</span>
            </h1>
            <p className="font-body-sm text-[14px] text-text-secondary">Kirim barang atau dokumen antar gedung/fakultas serta kos.</p>
          </div>
        </div>
      </div>
              ...
              <div className="lg:col-span-2 space-y-6">
                <form className="bg-surface-container rounded-2xl p-5 md:p-6 border border-outline-variant/30 shadow-lg space-y-4" onSubmit={handleOrder}>
                  ...
                  <div className="relative flex flex-col gap-4">
                    ...
                    {/* Titik Tujuan */}
                    <div className="relative z-10 space-y-2">
                      <label className="block font-label-mono text-[13px] text-on-surface-variant mt-6 ml-1">Tujuan & Penerima</label>
                      <div className="relative flex items-center">
                        <Image 
                          ...
                          className="absolute left-4"
                        />
                        <input type="text" required placeholder="Contoh: Titip ke Pak Satpam Fakultas Hukum atas nama Budi" className="w-full pl-11 pr-4 py-3 bg-surface-container-high border border-outline-variant/50 rounded-xl text-on-surface placeholder:text-outline/50 font-body-md text-[14px] focus:border-tertiary focus:outline-none" />
                      </div>
                    </div>
                  ...
                  <div className="space-y-1.5 pt-2">
                    ...
                    <textarea rows="2" placeholder="Contoh: Dokumen skripsi (Map Merah)." className="w-full p-4 bg-surface-container-high border border-outline-variant/50 rounded-xl text-on-surface placeholder:text-outline/50 font-body-md text-[14px] resize-none focus:border-tertiary focus:outline-none"></textarea>
                  </div>
                </form>
              </div>
                  ...
                  <div className="bg-surface-container rounded-2xl p-5 md:p-6 border border-outline-variant/30 shadow-lg flex flex-col h-full sticky top-24">
                    ...
                    <div className="mt-6 pt-4 border-t border-outline-variant/30">
                      ...
                      <button 
                        onClick={handleOrder} 
                        disabled={isLoading} 
                        className={`w-full py-3.5 bg-purple text-surface-container font-bold rounded-xl shadow-lg transition-all duration-300 font-headline-sm text-[16px] flex justify-center items-center gap-2 ${isLoading ? 'opacity-70 cursor-not-allowed' : 'hover:-translate-y-1 hover:shadow-[0_0_15px_rgba(173,200,245,0.4)] active:scale-95'}`}
                      >
                        {isLoading ? (
                      <>
                        <Image 
